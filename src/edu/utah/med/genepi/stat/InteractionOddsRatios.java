...
public class InteractionOddsRatios extends CCStatImp {

	//---------------------------------------------------------------------------
	public InteractionOddsRatios() 
	{
		title = "Interaction Odds Ratios, 2-Tailed Test";
	...
	public Result computeStat ( Analysis a, AnalysisTable[] tables, int[] simIndices )
	{
		...
		{
			boolean check = StatUt.checkRefCell( t.getRowFor(Ptype.CASE).getCells(), 
					        t.getRowFor(Ptype.CONTROL).getCells(),a.getRefColIndex());
			if ( check )
			{
				Result res = new  ResultImp.Real (
						StatUt.InteractionOddsRatios(
								t.getRowFor(Ptype.CASE).getCells(),
								t.getRowFor(Ptype.CONTROL).getCells(),
								a.getRefColIndex() ) );
				double[] resvals = res.doubleValues();
				for ( int i=0; i < resvals.length; i++ )
				{
					if ( resvals[i] == 0.0 )
					{
						return new ResultImp.StringResult("Interaction Odds Ratios 0.0");
					}
				}
				return res;
			}
